...
if (form) {
  form.addEventListener('submit', async function(e) {
    ...
    // Получаем значение honeypot (защита от ботов)
    ...
    try {
      const response = await fetch("https://api.formtomail.ru/send", {
        ...
        body: JSON.stringify({
          ...
          honeypot: honeypot // Передаем honeypot отдельно, не в body
        })
      });
      ...
      if (response.ok) {
        ...
        // Согласно документации, ошибку 400 стоит показать пользователю
        alert(`Ошибка отправки: ${result.message}`);
      } else {
        // Другие ошибки (например, 403 "Срок обслуживания истек")
